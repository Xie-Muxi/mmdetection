...
from .layers import OneFormerTransformerDecoder
...
from .transformer_decoder.oneformer_transformer_decoder import MLP
from .transformer_decoder.text_transformer import TextTransformer
from .transformer_decoder.transformer import Transformer
...
class OneFormerHead(Mask2FormerHead):
    ...
    def __init__(
        ...
        num_queries: int = 100,
        num_transformer_feat_level: int = 3,
        task_mlp: ConfigType = dict(
            input_dim=77,  # input.task_seq_len
            hidden_dim=256,  # oneformer.hidden_dim
            output_dim=256,  # oneformer.hidden_dim
            num_layer=2,
        ),  # from oneformer/config.py
        text_encoder: ConfigType = dict(
            context_length=77,  # text_encoder.width
            width=256,
            layers=12,
            vocab_size=49408,
        ),  # text encoder config
        text_projector: ConfigType = dict(
            input_dim=256, hidden_dim=256, output_dim=256, num_layers=2
        ),  # from oneformer_model.py
        prompt_ctx: ConfigType = dict(
            num_embeddings=16, embedding_dim=256
        ),  # from oneformer_model.py
        task: str = "panoptic",
        max_seq_len: int = 77,
        task_seq_len: int = 77,
        contrastive_multiScale_masked_transformer_decoder: ConfigType = dict(
            use_task_norm=True,
        ...
        ),  # from oneformer_transformer_decoder.py
        pixel_decoder: ConfigType = ...,
        enforce_decoder_input_project: bool = False,
        transformer_decoder: ConfigType = ...,
        positional_encoding: ConfigType = dict(num_feats=128, normalize=True),
        ...
        # use_task_norm: bool = True,  # from oneformer/config.py
        train_cfg: OptConfigType = None,
        test_cfg: OptConfigType = None,
        init_cfg: OptMultiConfig = None,
        **kwargs
        ...
        self.transformer_decoder = OneFormerTransformerDecoder(**transformer_decoder)
        ...
        self.decoder_norm = nn.LayerNorm(task_mlp.output_dim)
        self.decoder_input_projs = ModuleList()
        self.class_transformer = Transformer(**contrastive_multiScale_masked_transformer_decoder["class_transformer"])
        ...
        self.task = task
        self.contrastive_multiScale_masked_transformer_decoder = (
            contrastive_multiScale_masked_transformer_decoder
        )
        ...
        if train_cfg:
            ...
            # add extra config for text encoder and text projector
    ...
    def forward(
        ...
        # modify to add task and text
        tasks = torch.cat(
            [
                # self.task_tokenizer(x["task"]).to(self.device).unsqueeze(0)
                self.task_tokenizer(f'The task is {self.task}').to(self.device).unsqueeze(0)
                for x in batch_img_metas
            ],
            dim=0,
        )
        tasks = (
            self.task_tokenizer(tasks)
            .to(mask_features.device)
            .unsqueeze(0)
            .repeat(batch_size, 1)
        )
        ...
        tasks = tasks.unsqueeze(0)  # (1, batch_size, c)
        if self.prompt_ctx["use_task_norm"]:
            tasks = self.decoder_norm(tasks)

        texts_x = texts_x.unsqueeze(0)  # (1, batch_size, c)
        # if self.prompt_ctx["use_text_norm"]:
        #     texts_x = self.decoder_norm(texts_x)
        ...
        out_t, _ = self.class_transformer(
            ...
            tasks if self.prompt_ctx["use_task_norm"] else None,
        )
        out_t = out_t[0].permute(1, 0, 2)

        out = torch.cat(
            [out_t, tasks, texts_x] if self.training else [out_t, tasks], dim=0
        )
